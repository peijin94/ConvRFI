...
# with heaviside activation
    def forward(self, x):
        ...
        c3mask = torch.heaviside(F.conv2d(c3,
            torch.ones([2,1,5,5]),bias=None,stride=1,padding=2,groups=2),dum0)
...
def init_RFIconv(net,aggressive_factor = [1.6,1.65,0.5,0.5]):
    ...
    state_dict['conv3.weight'][0,0,:,:] = kernel_vertical[None,None,:,:]     # increase dimention of kernel
    state_dict['conv3.weight'][1,0,:,:] = kernel_horizontal[None,None,:,:]   # increase dimention of kernel
    state_dict['conv5.weight'][0,0,:,:] = kernel_left_mask[None,None,:,:]    # increase dimention of kernel
    state_dict['conv5.weight'][1,0,:,:] = kernel_right_mask[None,None,:,:]   # increase dimention of kernel
    state_dict['conv5.weight'][2,0,:,:] = kernel_upper_mask[None,None,:,:]   # increase dimention of kernel
    state_dict['conv5.weight'][3,0,:,:] = kernel_lower_mask[None,None,:,:]   # increase dimention of kernel
